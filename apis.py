...
async def join_chat(session: str, chat: str):
    c = Client('::memory::', in_memory=True, api_hash='408101818a2046b05b5fb9c3719f3c31', api_id=28679485,lang_code="ar", no_updates=True, session_string=session)
    try:
        await c.start()
    except:
        return False
...
async def reactions(session, link, like):
    client = Client('::memory::', in_memory=True, api_hash='408101818a2046b05b5fb9c3719f3c31', api_id=28679485,lang_code="ar", no_updates=True, session_string=session)
    await client.start()
...
async def check_chat(session: str, link: str):
    c = Client('::memory::', in_memory=True, api_hash='408101818a2046b05b5fb9c3719f3c31', api_id=28679485, no_updates=True, session_string=session)
    try:
        await c.start()
    except:
        return False
